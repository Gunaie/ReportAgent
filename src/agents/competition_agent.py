"""竞争分析 Agent — 公司对比、竞争格局、市场份额（v4-pro + 重试+降级）。"""
...
def _degraded_competition(topic: str, reason: str) -> str:
    """竞争分析不可用时的降级输出。"""
    return (
        f"## 竞争格局分析\n\n"
        f"> ⚠️ 竞争分析暂不可用: {reason}\n\n"
        f"### 建议\n"
        f"- v4-pro 模型可能响应较慢，稍后重试\n"
        f"- 研究主题: {topic}\n"
    )


async def competition_agent(state: ResearchState) -> dict:
    """竞争分析 — 使用 v4-pro 深度推理（内置重试）。"""
    topic = state["topic"]
    ...
    result = await safe_call(
        _call_llm,
        fallback=lambda: _degraded_competition(topic, "LLM调用失败"),
        name="竞争Agent",
        max_attempts=2,
        base_delay=1.5,
    )
    return {"competition_analysis": result}
